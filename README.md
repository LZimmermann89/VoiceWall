...
- **Systemweites Diktat:** globaler Hotkey (Standard
  `Cmd/Strg+Shift+D`, frei änderbar, Umschalt-Prinzip: einmal drücken
  startet, einmal drücken stoppt), Overlay "Ich höre zu" ohne Fokusklau,
  Live-Pegelanzeige, automatisches Einfügen an der Cursor-Position der
  Ziel-App (macOS per osascript, Windows per SendKeys), Kopieren-Knopf
  als Resilienzpfad: der Text geht nie verloren. Die Zwischenablage wird
  nach dem Einfügen wieder auf ihren vorherigen Inhalt zurückgesetzt
  (abschaltbar); hat der Nutzer zwischenzeitlich selbst etwas kopiert,
...
- **Deutsche Spracherkennung lokal:** whisper.cpp mit dem deutschen
  ...
  Erkennung läuft in einem eigenen Prozess und startet nach einem
  Absturz automatisch neu. Die Dekodierung ist deterministisch
  eingestellt, und Stille erzeugt garantiert keinen Text: das VAD
  entscheidet vor der Verschriftung, sodass die bekannten
  Whisper-Halluzinationen bei Ruhe nicht entstehen können. Auf macOS
  rechnet die GPU mit (Metal), unter Windows die CPU. Audio lebt
  ausschließlich im Arbeitsspeicher (Ringpuffer mit fester Obergrenze
  von zehn Minuten, aktiv genullt) und wird nie auf die Festplatte
...
- **Verwaltung (Ordner als Datenbank):** Diktate liegen als
  ...
  Tags mit Autocomplete und firmenweitem Umbenennen. Der Suchindex ist
  nur ein Lesecache: fehlt oder bricht er, baut ihn die App aus den
  Dateien selbst wieder auf. Die Dateien im Ordner sind immer die
  Wahrheit, nicht der Index.
...
- **Export:** Markdown mit Kopfdaten, Markdown ohne Kopfdaten, reiner
  Text und PDF (Prüfdokument-Layout, korrekte Umlaute nachgewiesen),
  einzeln oder als Stapel mit Fortschrittsanzeige; zusätzlich
  verschlüsselter Einzel-Export als `.vwenc` (AES-256-GCM, scrypt)
  samt Entschlüsselung in der App. Ein Export überschreibt nie eine
  vorhandene Datei.
- **Papierkorb:** Soft-Delete mit Wiederherstellen und endgültigem
  Löschen nach Bestätigung.
- **Modelle-Reiter:** alle Modelle im Überblick mit Zweck, Größe,
  SHA-256 und Speicherort. Modelle lassen sich hier gezielt
  nachladen und wieder löschen; das aktive Sprachmodell und die
  Sprachpausen-Erkennung sind gegen Löschen gesperrt, damit die App
  nicht funktionsunfähig gemacht werden kann.
