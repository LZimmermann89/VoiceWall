...
/**
 * Berechtigungs-Haertung (sichere Defaults):
...
if (!hasSingleInstanceLock) {
  ...
  app.on('second-instance', () => {
    // Fenster wieder oeffnen bzw. nach vorn holen (auch wenn es bei
  ...
  // Nach dem Onboarding (Einwilligung erteilt) lebt die App ohne Fenster
  // weiter: Tray und globaler Hotkey tragen das systemweite Diktat.
  // Vor dem Onboarding gibt es weder Hotkey-Nutzen noch Tray-Erwartung,
  // dann beendet das Schliessen des Fensters die App.
  ...
  void app.whenReady().then(async () => {
    ...
    // eine zentrale Stelle beim Start; danach aktualisiert
    ...
    // Firmenverwaltung (Ordner-als-Datenbank). Der Desktop wird pro
    ...
    // Konfig-Schreiber: FlowController UND CompanyManager teilen sich
    ...
    companyManager = new CompanyManager({
      ...
      // Firmen-/Sprachwechsel aendert Modellbedarf und Status.
      onCompanyChanged: () => {
    ...
    // Diktat-Kontext: der Orchestrator holt sich vor
    ...
    // Frisch registrierte Statusanteile (Hotkey, Accessibility) anzeigen.
    orchestrator.notifyStatusChanged();

    // Ready-Marker fuer das Bootstrap-Skript:
