...
export interface CompanyManagerDeps {
   ...
   * der Orchestrator aktualisiert damit Status/Modellbedarf.
   */
  readonly onCompanyChanged?: () => void;
  /**
   * Zentraler, serialisierter Konfig-Schreibpfad. Im App-Bootstrap
 ...
 * aller Steuerzeichen (konsistent zur clean()-Baseline).
 */
...
export class CompanyManager {
  ...
  /** Serialisierter Schreibpfad: injiziert oder eigene Instanz. */
   ...
   * Writer: frisch lesen, Mutator anwenden, atomar schreiben. Der
  ...
  /**
   * Diktatsprache der aktiven Firma: bestimmt Modell und
  ...
  /**
   * Diktat-Kontext fuer den Orchestrator: Sprache der aktiven
   ...
   * Mit `volltext: true` werden zusaetzlich die
  ...
  // ------------------------------------------------------------------
  // Verwaltungs-UI: Detail, Bearbeiten, Notiz, Tags, Export,
  ...
  /**
   * Stapel-Export: mehrere Diktate als Markdown/TXT/PDF. Bei mehr als
  ...
  /** Tag-Batch-Rename: wirkt firmenweit inklusive Papierkorb. */
  ...
  /**
   * Verschluesselter Einzel-Export: Markdown mit Front-Matter,
  ...
  /**
   * Entschluesselt eine .vwenc-Datei: die Datei waehlt der Nutzer im
  ...
  // ------------------------------------------------------------------
  // Fach-Woerterbuch (Stufe 1): vokabular.json der
  ...
  register(): void {
    ...
    // Diktatsprache der aktiven Firma nachtraeglich wechseln.
