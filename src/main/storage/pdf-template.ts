/**
 * Druck-HTML-Vorlage des PDF-Exports.
 *
 ...
 *
 * Sprache: die PDF-Beschriftungen folgen der UI-Sprache zum Exportzeitpunkt
 * (Katalog via src/main/i18n.ts, ohne Electron-Import).
 *
 * Sicherheit (Stored-XSS-Regel): Titel, Metadaten und Body
...
/** HTML-Escaping fuer Textknoten und Attributwerte (Output-Encoding). */
 ...
 * ("... Uhr" auf Deutsch) kommt aus dem Katalog der UI-Sprache.
 */
...
export function buildPrintHtml(meta: TranscriptMeta, body: string): string {
  // PDF-Sprache = UI-Sprache zum EXPORTZEITPUNKT.
  const t = texte().pdf;
