/**
 * Stapel-Export: mehrere Diktate in einem Zug nach `Exporte/` exportieren.
 *
 * - GENAU EINE Datei: normaler Einzel-Export (gleiche Pfade wie beim
 *   Einzel-Export aus export.ts).
 ...
 * - ZIP bewusst NICHT: Node bringt kein ZIP mit und eine
...
export async function exportTranscriptsBatch(
  ...
  try {
    for (const [index, relPfad] of pfade.entries()) {
      ...
      // Einzelsegment-Containment auch fuer jeden Stapel-Dateinamen.
