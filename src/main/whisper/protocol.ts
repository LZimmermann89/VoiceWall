...
export const workerCommandSchema = z.discriminatedUnion('type', [
  z.object({
    ...
    /**
     * Sprache der Oberflaeche: der Worker waehlt damit seine
   ...
   * deterministisch auf das letzte Segment warten kann (Hotkey-Stop).
   */
  z.object({ type: z.literal('flush'), requestId: z.string().optional() }),
  /** Akkumuliertes Segment ohne Transkription verwerfen. */
  z.object({ type: z.literal('reset') }),
  /**
   * Diktat-Kontext fuer alle folgenden Transkriptionen setzen (Stufe 1):
   * `language` ist die feste Diktatsprache der aktiven Firma
  ...
  z.object({
    ...
    /** Sprache der Oberflaeche, reist mit jedem Kontext mit. */
    uiLanguage: z.enum(['de', 'en']).optional(),
  }),
  /**
