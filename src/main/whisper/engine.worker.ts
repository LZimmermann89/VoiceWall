 ...
 *
 * Verortung: Whisper und Silero-VAD laufen hier im isolierten
 * utilityProcess, nie im Main-Prozess und nie im Renderer. Ein
...
/**
 * Diktat-Kontext (Stufe 1), gesetzt per `set-context`:
...
/**
 * Sprache der Oberflaeche: reist mit `init` und `set-context`
...
function handleCommand(command: WorkerCommand): void {
  switch (command.type) {
    ...
    case 'set-context':
      ...
      // Die UI-Sprache wirkt sofort (betrifft nur Fehlertexte).
      if (command.uiLanguage !== undefined) {
        uiLanguage = command.uiLanguage;
      }
      enqueue(() => {
