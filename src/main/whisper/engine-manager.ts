...
/** Sinnvolle Standardwerte fuer Endpointing/VAD. */
...
export class WhisperEngineManager {
  ...
  private restarts = 0;
  /**
   * Zuletzt gesetzter Diktat-Kontext (Sprache plus Initial-Prompt,
   * Stufe 1). Wird nach einem Engine-Neustart automatisch erneut
   ...
   * passieren: solche Zeilen koennen Transkripttext enthalten und werden
   * deshalb NIE persistiert (siehe native-log.ts).
   */
  ...
  private spawnAndInit(): Promise<Result<void, string>> {
    return new Promise((resolve) => {
      ...
      // RAM-Puffer, weil Randfall-Zeilen Transkripttext tragen koennen.
      ...
      this.post({
        ...
        // UI-Sprache fuer die wenigen nutzersichtbaren Worker-Texte.
  ...
  setContext(context: DictationContext): void {
    ...
    this.post({
      ...
      // Sprache reist im Kontext mit: der Worker uebersetzt damit
  ...
  /**
   * Nur Dev/Test (Prompt-Beweis): der zuletzt an den Worker gesendete
