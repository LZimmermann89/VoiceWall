/**
 * Clipboard-Sequenz der Diktat-Zustellung (Datenschutzmassnahme, Risiko R7).
 *
 * Ablauf:
 ...
 * bewusst nicht eingefuehrt). Die wirksame R7-Massnahme ist stattdessen die
 * hier implementierte sofortige Wiederherstellung/Ueberschreibung.
 * Details: docs/ENTSCHEIDUNGEN.md.
 *
...
export async function runClipboardSequence(
  ...
  const restore = (async (): Promise<RestoreOutcome> => {
    ...
    if (!options.restorePrevious) {
      ...
      // Datenschutzmassnahme, siehe docs/ENTSCHEIDUNGEN.md).
      return 'disabled';
    }
