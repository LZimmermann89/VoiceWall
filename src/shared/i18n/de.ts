...
export const de = {
  ...
  diktat: {
    ...
    zielanwendungLabel:
      'Zielanwendung am Satzende erkennen: endet das Diktat auf "Schicke das an Word", "an Word senden" oder "Füge das in Excel ein", wird der Befehl entfernt, das genannte Fenster nach vorne geholt und der Text dort eingefügt. Präposition und Verb sind Pflicht, deshalb löst ein Satz, der nur auf einen Programmnamen endet ("die Daten in Excel"), nichts aus. Läuft das Programm nicht, wird nichts eingefügt: der Text bleibt in der Zwischenablage und eine Meldung nennt den Grund. Erkannt werden beide Sprachstellungen, "Schicke das an Word" ebenso wie "an Word senden". Verben: senden, schicken, einfügen, übertragen, kopieren, packen; Einleitungen: an, in, ins, nach, zu. Vor dem Befehl muss Text stehen, sonst gäbe es nichts zuzustellen. Eine Kommandozeile ist bewusst nicht ansteuerbar. Standardmäßig aus.',
