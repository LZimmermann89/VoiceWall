/**
 * English UI catalog. Typed as `Uebersetzung = typeof de`:
...
export const en: Uebersetzung = {
  ...
  diktat: {
    ...
    zielanwendungLabel:
      'Detect a target application at the end of a sentence: if the dictation ends with "Send this to Word", "to Word send" or "Insert this into Excel", that phrase is removed, the named window is brought to the front and the text is inserted there. Preposition and verb are both required, so a sentence merely ending in a program name ("the figures in Excel") triggers nothing. If the program is not running, nothing is inserted: the text stays in the clipboard and a message explains why. Both word orders are recognized, "Send this to Word" as well as "to Word send". Verbs: send, insert, paste, transfer, copy, put; introductions: to, into. There must be text before the command, otherwise there would be nothing to deliver. A command line is deliberately not available as a target. Off by default.',
  ...
  /** Immediately visible notices (toast system). */
  toast: {
    fehlerKicker: 'Error',
  ...
  /**
   * User-visible texts of the MAIN process:
   * Result error messages, tray, overlay delivery messages, PDF template
   * and model display names. Logs intentionally remain German.
   */
  main: {
    generisch: {
