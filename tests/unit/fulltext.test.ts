 ...
 *   die dokumentierte Grundlage fuer "kein Volltext-Cache".
 */
...
describe('searchTranscriptBodies', () => {
  ...
  it(`PERFORMANCE: ${String(GESAMT)} Diktate in unter 500 ms durchsucht`, async () => {
    ...
    expect(treffer.size).toBe(1);
    // Dokumentierte Messgrundlage der Cache-Entscheidung. Auf
    ...
    console.log(
      `Volltextsuche ueber ${String(GESAMT)} Diktate: ${dauerMs.toFixed(1)} ms (Schwelle ${String(schwelleMs)} ms, kein Cache)`,
    );
