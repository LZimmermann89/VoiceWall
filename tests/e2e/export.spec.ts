/**
 * E2E-Tests der Export- und Suchfunktionen:
 *
 * 1. PDF-Export: ein Diktat mit "Ä Ö Ü ä ö ü ß" in Titel
 ...
 *    Text als komprimierte Font-Subset-Glyphen schreibt.
...
test('PDF-Export bettet echte Umlaute korrekt ein (Text-Extraktion)', async () => {
  ...
  try {
    ...
    // Beweis: Umlaute stehen als echter Text IM PDF (Titel und Body).
...
test('Stapel-Export: 2 von 3 Eintraegen ausgewaehlt landen im Stapel-Ordner', async () => {
  const { app, window } = await launchApp({ withCompany: true });
  try {
...
test('Volltextsuche findet einen Begriff, der NUR im Body steht', async () => {
  const { app, window } = await launchApp({ withCompany: true });
  try {
...
test('Verschluesselter Export erzeugt eine .vwenc-Datei ohne Klartext', async () => {
  const { app, window } = await launchApp({ withCompany: true });
  try {
