...
test('Register, Suche, Detail, Bearbeiten/Tag, Export, Soft-Delete/Wiederherstellen', async () => {
...
test('Firmenwechsel laedt den getrennten Bestand (zwei Firmen)', async () => {
  const { app, window } = await launchApp({ withCompany: true });
  try {
...
test('XSS-Probe: HTML-artiger Body wird als Text angezeigt (kein img, kein Skript)', async () => {
  const { app, window } = await launchApp({ withCompany: true });
  try {
    await expect(window.locator('h1')).toContainText('VoiceWall');
...
test('Beleg-Ansicht zeigt die Anbieterkennzeichnung (Impressum, § 5 DDG) lokal an', async () => {
  const { app, window } = await launchApp({ withCompany: true });
  try {
    await expect(window.locator('h1')).toContainText('VoiceWall');
    ...
    // Der Quelle-Knopf existiert und traegt die statische URL (dokumentierte
    // openExternal-Ausnahme); er wird hier bewusst NICHT geklickt (kein
    // Browser im Test).
