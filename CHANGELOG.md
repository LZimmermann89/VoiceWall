...
- README (deutsch und englisch) auf den tatsächlichen Funktionsstand
  gebracht: der Modelle-Reiter (Nachladen, Löschen, Prüfsummen, Sperre
  für aktives Modell und Sprachpausen-Erkennung) war bisher gar nicht
  beschrieben. Ergänzt wurden außerdem die Wiederherstellung der
  Zwischenablage samt Schutz vor Überschreiben eigener Kopien, der
  Abbruch bei Sperrbildschirm und Ruhezustand, die Live-Pegelanzeige,
  der frei änderbare Hotkey, der eigene Prozess für die Erkennung mit
  Neustart nach Absturz, die deterministische Dekodierung, die
  VAD-Schleuse gegen Halluzinationen bei Stille, Metal auf macOS, die
  Zehn-Minuten-Grenze des RAM-Ringpuffers, die Selbstheilung des
  Suchindex aus den Dateien sowie die vollständige Liste der
  Export-Varianten (Markdown mit und ohne Kopfdaten) inklusive der
  Zusage, dass ein Export nie eine vorhandene Datei überschreibt.

- Interne Planungs-Kürzel vollständig aus dem Repo entfernt: Kommentare,
  Testnamen und Konfigurationstexte verweisen nicht mehr auf
  Unterlagen, die nicht Teil dieses Repos sind. Der Inhalt der
  Kommentare bleibt unverändert, nur der Verweis fällt weg. Damit
  entfällt auch der bisherige README-Hinweis auf diese Kürzel.
...
- fp16-Selbst-Hash-Quercheck erledigt: die vollständig geladene
...
- Installer, Packaging und First-Run-Wizard. Packaging als reiner
  ...
  First-Run-Wizard als echte App-Shell (ersetzt die frühere Test-UI):
